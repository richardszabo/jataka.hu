  ...
  while(1) {
	...
	if(see.ball.pos.distance<=0.5) {
	  ...
	  if(see.goal[DIR_FD].time==see.time) {
	    ...
	    int bestdir = 0;
	    for(i=0; i<18; ++i) {
	      if(fitness[i]>bestdir) {
		bestdir = fitness[i];
		bestidx = i;
	      } else if(fitness[i]==bestdir && abs(hotidx-i)<abs(hotidx-bestidx)) {
		bestidx = i;
	      }
	    }
	    RCLOG("I pass or I shoot\n");
