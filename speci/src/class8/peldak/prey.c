...
/* Version:      1.0                                                         */
/* Date:         07-Oct-98                                                   */
/* Description:  Prey in the prey - predator experiment.                     */
...
void load()
{
  ...
  if (file==NULL) fprintf(stderr,"prey: %s file not found\n",filename);
...
void create_gui()
{
  window = gui_window_new("Prey",30,30,160,100);
...
int main()
{
  ...
  khepera_live();
  /* create_gui(); */ /* uncomment this to have a graphical user interface */
  load();
  create_network();
